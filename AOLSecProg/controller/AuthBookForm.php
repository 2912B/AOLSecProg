<?php
    ...
    function regisBuku($db, $judul, $tahun, $genre, $halaman){
        ...
        header("Location: ../homeuser.php?error=none");
        exit();
    }
    ...
    if($_SERVER['REQUEST_METHOD'] === "POST"){
        ...
        if(!preg_match("/^[a-zA-Z0-9]*$/", $judul)){
            header("Location: ../publishform.php?error=invalidtitle");
            exit();
        }else if(checklength($judul, 40) !== false){
            header("Location: ../publishform.php?error=mustnotexceed40chars");
            exit();
        }

        // check tahun
        if(strlen($tahun) !== 4){ 
            header("Location: ../publishform.php?error=yearmustbe4chars");
            exit();
        }else if(!preg_match("/^[0-9]*$/", $tahun)){
            header("Location: ../publishform.php?error=yearmustbenumbers");
            exit();
        }

        // check genre
        if(checklength($genre, 20) !== false){
            header("Location: ../publishform.php?error=mustnotexceed20chars");
            exit();
        }

        // check halaman
        if(!preg_match("/^[0-9]*$/", $halaman)){
            header("Location: ../publishform.php?error=invalidhalaman");
            exit();
        }
